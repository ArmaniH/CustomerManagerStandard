(function () {
    ...
    var OrderEditController = function ($scope, $location, $routeParams,
                                           $timeout, config, dataService, modalService) {
        ...
        vm.saveOrder = function () {
            if ($scope.editForm.$valid) {
                if (!vm.customer.id) {
                    dataService.insertOrder(vm.customer).then(processSuccess, processError);
                }
                else {
                    dataService.updateOrder(vm.customer).then(processSuccess, processError);
                }
            }
        };
        ...
        vm.deleteOrder = function () {
            ...
            modalService.showModal({}, modalOptions).then(function (result) {
                if (result === 'ok') {
                    dataService.deleteOrder(vm.customer.id).then(function () {
                        onRouteChangeOff(); //Stop listening for location changes
                        $location.path('/customersorders');
                    }, processError);
                }
            });
        };
        ...
        function init() {
            ...
            getStates().then(function () {
                if (customerId > 0) {
                    dataService.getOrder(customerId).then(function (customer) {
                        vm.customer = customer;
                    }, processError);
                } else {
                    dataService.newOrder().then(function (customer) {
                        vm.customer = customer;
                    });
                }
            });
    ...
    angular.module('customersApp').controller('OrderEditController', OrderEditController);
